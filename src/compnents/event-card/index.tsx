      ...
      <div className={`row ${style.event_row}`}>
        ...
        <div className={`col-8 col-sm-10 ${style.event_data_wrapper}`}>
          <div className={`${style.event_head}`}>
            <div className={`${style.event_title}`}>{data.title}</div>
            {data?.upcoming && (
              <span className={`${style.upcoming_event}`}>Upcoming Event</span>
            )}
          </div>
          ...
          <div className={`${style.event_detail}`}>
            <span className={`${style.event_author}`}>{data.author}</span>
            <span className={`${style.event_dop}`}>{data.date}</span>
          </div>
        </div>
      </div>
    </div>
